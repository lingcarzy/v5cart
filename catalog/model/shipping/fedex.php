...
class ModelShippingFedex extends Model {
	function getQuote($address) {
		...
		$query = $this->db->query("SELECT * FROM @@zone_to_geo_zone WHERE geo_zone_id = '" . (int)C('fedex_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");
	
		if (!C('fedex_geo_zone_id')) {
			$status = true;
		...
		if ($status) {
			$weight = $this->weight->convert($this->cart->getWeight(), C('config_weight_class_id'), C('fedex_weight_class_id'));
			$weight_code = strtoupper($this->weight->getUnit(C('fedex_weight_class_id')));
			...
			$country_info = $this->model_localisation_country->getCountry(C('config_country_id'));
			
			M('localisation/zone');
			
			$zone_info = $this->model_localisation_zone->getZone(C('config_zone_id'));
			...
			$xml .= '					<ns1:Key>' . C('fedex_key') . '</ns1:Key>';
			$xml .= '					<ns1:Password>' . C('fedex_password') . '</ns1:Password>';
			$xml .= '				</ns1:UserCredential>';
			$xml .= '			</ns1:WebAuthenticationDetail>';
			$xml .= '			<ns1:ClientDetail>';
			$xml .= '				<ns1:AccountNumber>' . C('fedex_account') . '</ns1:AccountNumber>';
			$xml .= '				<ns1:MeterNumber>' . C('fedex_meter') . '</ns1:MeterNumber>';
			$xml .= '			</ns1:ClientDetail>';
			$xml .= '			<ns1:Version>';
			...
			$xml .= '				<ns1:DropoffType>' . C('fedex_dropoff_type') . '</ns1:DropoffType>';	
			$xml .= '				<ns1:PackagingType>' . C('fedex_packaging_type') . '</ns1:PackagingType>';					
			$xml .= '				<ns1:Shipper>';
			$xml .= '					<ns1:Contact>';
            $xml .= '						<ns1:PersonName>' . C('config_owner') . '</ns1:PersonName>';
            $xml .= '						<ns1:CompanyName>' . C('config_name') . '</ns1:CompanyName>';
            $xml .= '						<ns1:PhoneNumber>' . C('config_telephone') . '</ns1:PhoneNumber>';
          	$xml .= '					</ns1:Contact>';
			$xml .= '					<ns1:Address>';
			...
			$xml .= '						<ns1:PostalCode>' . C('fedex_postcode') . '</ns1:PostalCode>';
            ...
            $xml .= '						<ns1:CountryCode>' . $country_info['iso_code_2'] . '</ns1:CountryCode>';
          	$xml .= '					</ns1:Payor>';
			$xml .= '				</ns1:ShippingChargesPayment>';
			$xml .= '				<ns1:RateRequestTypes>' . C('fedex_rate_type') . '</ns1:RateRequestTypes>';
			...
			if ($dom->getElementsByTagName('HighestSeverity')->item(0)->nodeValue == 'FAILURE' || $dom->getElementsByTagName('HighestSeverity')->item(0)->nodeValue == 'ERROR') {
				...
				foreach ($rate_reply_details as $rate_reply_detail) { 
					...
					if (in_array(strtoupper($code), C('fedex_service'))) {
						$title = L('text_' . $code);
						
						if (C('fedex_display_time')) {
							$title .= ' (' . L('text_eta') . ' ' . date(L('date_format_short') . ' ' . L('time_format'), strtotime($rate_reply_detail->getElementsByTagName('DeliveryTimestamp')->item(0)->nodeValue)) . ')';
						}
						...
						$quote_data[$code] = array(
							...
							'tax_class_id' => C('fedex_tax_class_id'),
							'text'         => $this->currency->format($this->tax->calculate($this->currency->convert($cost, $currency, $this->currency->getCode()), C('fedex_tax_class_id'), C('config_tax')), $this->currency->getCode(), 1.0000000)
						);
					}
				}
			}
		...
		if ($quote_data || $error) {
			...
			if (C('fedex_display_weight')) {
				$title .= ' (' . L('text_weight') . ' ' . $this->weight->format($weight, C('fedex_weight_class_id')) . ')';
			}
