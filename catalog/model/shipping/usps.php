...
class ModelShippingUsps extends Model {
	public function getQuote($address) {
		...
		$query = $this->db->query("SELECT * FROM @@zone_to_geo_zone WHERE geo_zone_id = '" . (int)C('usps_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

		if (!C('usps_geo_zone_id')) {
			$status = true;
		...
		if ($status) {
			...
			$weight = $this->weight->convert($this->cart->getWeight(), C('config_weight_class_id'), C('usps_weight_class_id'));
			...
			if ($address['iso_code_2'] == 'US') {
				$xml  = '<RateV4Request USERID="' . C('usps_user_id') . '">';
				$xml .= '	<Package ID="1">';
				$xml .=	'		<Service>ALL</Service>';
				$xml .=	'		<ZipOrigination>' . substr(C('usps_postcode'), 0, 5) . '</ZipOrigination>';
				...
				if (C('usps_container') == 'RECTANGULAR' && C('usps_size') == 'REGULAR') {
					$this->config->set('usps_container', 'VARIABLE');
				}

				$xml .=	'		<Container>' . C('usps_container') . '</Container>';
				$xml .=	'		<Size>' . C('usps_size') . '</Size>';
				$xml .= '		<Width>' . C('usps_width') . '</Width>';
				$xml .= '		<Length>' . C('usps_length') . '</Length>';
				$xml .= '		<Height>' . C('usps_height') . '</Height>';
				
				// Calculate girth based on usps calculation
				$xml .= '		<Girth>' . (round(((float)C('usps_length') + (float)C('usps_width') * 2 + (float)C('usps_height') * 2), 1)) . '</Girth>';
				 
				 
				$xml .=	'		<Machinable>' . (C('usps_machinable') ? 'true' : 'false') . '</Machinable>';
				$xml .=	'	</Package>';
				$xml .= '</RateV4Request>';
				...
				if (isset($country[$address['iso_code_2']])) {
					$xml  = '<IntlRateV2Request USERID="' . C('usps_user_id') . '">';
					$xml .=	'	<Package ID="1">';
					$xml .=	'		<Pounds>' . $pounds . '</Pounds>';
					$xml .=	'		<Ounces>' . $ounces . '</Ounces>';
			...
			if ($status) {
					...
					if ($rate_response || $intl_rate_response) {
						if ($address['iso_code_2'] == 'US') {
										...
										if ($classid == '0') {
											...
											if ((C('usps_domestic_' . $classid))) {
												...
												$quote_data[$classid] = array(
													...
													'cost'         => $this->currency->convert($cost, 'USD', C('config_currency')),
													'tax_class_id' => C('usps_tax_class_id'),
													'text'         => $this->currency->format($this->tax->calculate($this->currency->convert($cost, 'USD', $this->currency->getCode()), C('usps_tax_class_id'), C('config_tax')), $this->currency->getCode(), 1.0000000)
												);
											}

										} elseif (C('usps_domestic_' . $classid)) {
											$cost = $postage->getElementsByTagName('Rate')->item(0)->nodeValue;
							...
							foreach ($services as $service) {
								...
								if (in_array($id, $allowed) && C('usps_international_' . $id)) {
									$title = $service->getElementsByTagName('SvcDescription')->item(0)->nodeValue;

									if (C('usps_display_time')) {
										$title .= ' (' . L('text_eta') . ' ' . $service->getElementsByTagName('SvcCommitments')->item(0)->nodeValue . ')';
									}
	  		...
	  		if ($quote_data) {
				...
				if (C('usps_display_weight')) {
					$title .= ' (' . L('text_weight') . ' ' . $this->weight->format($weight, C('usps_weight_class_id')) . ')';
				}
