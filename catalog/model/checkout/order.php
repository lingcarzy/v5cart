...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		foreach ($data['products'] as $product) {
			...
			$order_product_id = $this->db->insert('order_product', $product);
			...
			foreach ($product['download'] as $download) {
				...
				$download['remaining'] = (int)($download['remaining'] * $product['quantity']);
	...
	public function getOrder($order_id) {
		$order = $this->db->queryOne("SELECT *, (SELECT os.name FROM `@@order_status` os WHERE os.order_status_id = o.order_status_id AND os.language_id = o.language_id) AS order_status FROM `@@order` o WHERE o.order_id = " . (int)$order_id);

		if (!$order) return false;

		$country = $this->db->queryOne("SELECT * FROM `@@country` WHERE country_id = " . $order['payment_country_id']);
		...
		$zone = $this->db->queryOne("SELECT * FROM `@@zone` WHERE zone_id = " . $order['payment_zone_id']);

		if ($zone) {
			$order['payment_zone_code'] = $zone['code'];
		} else {
			$order['payment_zone_code'] = '';
		}

		$country = $this->db->queryOne("SELECT * FROM `@@country` WHERE country_id = " . $order['shipping_country_id']);
		...
		$zone = $this->db->queryOne("SELECT * FROM `@@zone` WHERE zone_id = " . $order_query->row['shipping_zone_id']);

		if ($zone) {
			$order['shipping_zone_code'] = $zone['code'];
		} else {
		...
		if ($language_info) {
			...
			$order['language_filename'] = $language_info['filename'];
			$order['language_directory'] = $language_info['directory'];
		} else {
			$order['language_code'] = '';
			$order['language_filename'] = '';
			$order['language_directory'] = '';
		}
	...
	public function confirm($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		if ($order_info && !$order_info['order_status_id']) {
			// Fraud Detection
			if (C('config_fraud_detection')) {
				M('checkout/fraud');

				$risk_score = $this->model_checkout_fraud->getFraudScore($order_info);

				if ($risk_score > C('config_fraud_score')) {
					$order_status_id = C('config_fraud_status_id');
				}
			}
			...
			if ($status) {
				$order_status_id = C('config_order_status_id');
			}

			$this->db->query("UPDATE `@@order` SET order_status_id = " . (int)$order_status_id . ", date_modified = NOW() WHERE order_id = " . (int)$order_id);

			$this->db->query("INSERT INTO @@order_history SET order_id = " . (int)$order_id . ", order_status_id = " . (int)$order_status_id . ", notify = 1, comment = '" . ES(($comment && $notify) ? $comment : '') . "', date_added = NOW()");
			...
			if (C('config_complete_status_id') == $order_status_id) {
			...
			$template->data['logo'] = C('config_url') . 'image/' . C('config_logo');
			...
			if (file_exists(DIR_TEMPLATE . C('config_template') . '/template/mail/order.tpl')) {
				$html = $template->fetch(C('config_template') . '/template/mail/order.tpl');
			} else {
			...
			foreach ($order_product_query->rows as $product) {
				$text .= $product['quantity'] . 'x ' . $product['name'] . ' (' . $product['model'] . ') ' . html_entity_decode($this->currency->format($product['total'] + (C('config_tax') ? ($product['tax'] * $product['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value']), ENT_NOQUOTES, 'UTF-8') . "\n";
			...
			// Admin Alert Mail
			if (C('config_alert_mail')) {
				$subject = sprintf($language->get('text_new_subject'), html_entity_decode(C('config_name'), ENT_QUOTES, 'UTF-8'), $order_id);
	...
	public function update($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		if ($order_info && $order_info['order_status_id']) {
			...
			$this->db->query("INSERT INTO @@order_history SET order_id = " . (int)$order_id . ", order_status_id = " . (int)$order_status_id . ", notify = " . (int)$notify . ", comment = '" . ES($comment) . "', date_added = NOW()");
