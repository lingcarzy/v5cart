...
class ControllerCommonContentTop extends Controller {
	protected function index() {
		...
		foreach ($extensions as $extension) {
			...
			if ($modules) {
				foreach ($modules as $module) {
					if ($module['layout_id'] == LAYOUT && $module['position'] == 'content_top' && $module['status']) {
						$module_data[] = array(
							'code'       => $extension,
							'setting'    => $module,
		...
		foreach ($module_data as $module) {
			$cache = $module['cache'] ? C('config_store_id') . $module['code'] . '.' . $module['id'] . 'T'. LAYOUT : FALSE;
		...
		$this->render('common/content_top.tpl');
	}
}
?>