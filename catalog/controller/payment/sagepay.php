...
class ControllerPaymentSagepay extends Controller {
	protected function index() {
		...
		$data['Description'] = sprintf(L('text_description'), date(L('date_format_short')), $this->session->data['order_id']);
		$data['SuccessURL'] = str_replace('&amp;', '&', U('payment/sagepay/success', 'order_id=' . $this->session->data['order_id']));
		$data['FailureURL'] = str_replace('&amp;', '&', U('checkout/checkout', '', 'SSL'));
		...
		$data['VendorEMail'] = C('config_email');
		...
		if (!C('sagepay_transaction')) {
			$data['ApplyAVSCV2'] = '0';
		}

 		$data['Apply3DSecure'] = '0';

		$this->data['transaction'] = C('sagepay_transaction');
		...
		$this->render('payment/sagepay.tpl');
	}
	...
	public function success() {
		if (isset($this->request->get['crypt'])) {
			...
			if ($data && is_array($data)) {
				...
				$this->model_checkout_order->confirm($this->request->get['order_id'], C('config_order_status_id'));
				...
				if ($data['Status'] == 'OK') {
					$this->model_checkout_order->update($this->request->get['order_id'], C('sagepay_order_status_id'), $message, false);
				} else {
					$this->model_checkout_order->update($this->request->get['order_id'], C('config_order_status_id'), $message, false);
				}
