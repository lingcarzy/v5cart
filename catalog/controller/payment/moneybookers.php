...
class ControllerPaymentMoneybookers extends Controller {
	protected function index() {
		...
		$this->data['action'] = 'https://www.moneybookers.com/app/payment.pl?p=V5Cart';

		$this->data['pay_to_email'] = C('moneybookers_email');
		$this->data['platform'] = '31974336';
		$this->data['description'] = C('config_name');
		$this->data['transaction_id'] = $this->session->data['order_id'];
        $this->data['return_url'] = U('checkout/success');
		$this->data['cancel_url'] = U('checkout/checkout', '', 'SSL');
		$this->data['status_url'] = U('payment/moneybookers/callback');
		$this->data['language'] = $this->session->data['language'];
		$this->data['logo'] = C('config_url') . 'image/' . C('config_logo');
		...
		$this->render('payment/moneybookers.tpl');
	}
		...
		if ($order_info) {
			...
			if ($verified) {
				switch($this->request->post['status']) {
					case '2':
						$this->model_checkout_order->update($order_id, C('moneybookers_order_status_id'), '', true);
						break;
					case '0':
						$this->model_checkout_order->update($order_id, C('moneybookers_pending_status_id'), '', true);
						break;
					case '-1':
						$this->model_checkout_order->update($order_id, C('moneybookers_canceled_status_id'), '', true);
						break;
					case '-2':
						$this->model_checkout_order->update($order_id, C('moneybookers_failed_status_id'), '', true);
						break;
					case '-3':
						$this->model_checkout_order->update($order_id, C('moneybookers_chargeback_status_id'), '', true);
						break;
				}
			} else {
				$this->log->write('md5sig returned (' + $md5sig + ') does not match generated (' + $md5hash + '). Verify Manually. Current order state: ' . C('config_order_status_id'));
			}
		}
	}
}
