...
class ControllerPaymentAuthorizeNetAim extends Controller {
	protected function index() {
		...
		$this->render('payment/authorizenet_aim.tpl');
	}

	public function send() {
		if (C('authorizenet_aim_server') == 'live') {
    		$url = 'https://secure.authorize.net/gateway/transact.dll';
		} elseif (C('authorizenet_aim_server') == 'test') {
		...
		$data['x_login'] = C('authorizenet_aim_login');
		$data['x_tran_key'] = C('authorizenet_aim_key');
		$data['x_version'] = '3.1';
		...
		$data['x_description'] = html_entity_decode(C('config_name'), ENT_QUOTES, 'UTF-8');
		...
		$data['x_method'] = 'CC';
		$data['x_type'] = (C('authorizenet_aim_method') == 'capture') ? 'AUTH_CAPTURE' : 'AUTH_ONLY';
		...
		if (C('authorizenet_aim_mode') == 'test') {
		...
		if (curl_error($curl)) {
			...
			if ($response_info[1] == '1') {
				if (strtoupper($response_info[38]) == strtoupper(md5(C('authorizenet_aim_hash') . C('authorizenet_aim_login') . $response_info[7] . $this->currency->format($order_info['total'], $order_info['currency_code'], 1.00000, false)))) {
					...
					$this->model_checkout_order->update($this->session->data['order_id'], C('authorizenet_aim_order_status_id'), $message, false);
				}

				$json['success'] = U('checkout/success', '', 'SSL');
			} else {
