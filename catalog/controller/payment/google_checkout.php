...
class ControllerPaymentGoogleCheckout extends Controller {
	public function index() {
		...
		if (!C('google_checkout_test')) {
			$this->data['action'] = 'https://checkout.google.com/api/checkout/v2/checkout/Merchant/' . C('google_checkout_merchant_id');	
		} else {
			$this->data['action'] = 'https://sandbox.google.com/checkout/api/checkout/v2/checkout/Merchant/' . C('google_checkout_merchant_id');
		}
			
		$this->data['merchant'] = C('google_checkout_merchant_id');
		
		$this->render('payment/google_checkout.tpl');
	}
	...
	public function send() {
		...
		//	$xml .= '   	</merchant-private-data>'; 
			...
			$key = C('google_checkout_merchant_key');
			$blocksize = 64;
			$hash = 'sha1';
