...
class ControllerPaymentPPPro extends Controller {
	protected function index() {
		...
		$this->render('payment/pp_pro.tpl');
	}

	public function send() {
		if (!C('pp_pro_transaction')) {
		...
		$request .= '&USER=' . urlencode(C('pp_pro_username'));
		$request .= '&PWD=' . urlencode(C('pp_pro_password'));
		$request .= '&SIGNATURE=' . urlencode(C('pp_pro_signature'));
		...
		if (($response_info['ACK'] == 'Success') || ($response_info['ACK'] == 'SuccessWithWarning')) {
			...
			$this->model_checkout_order->update($this->session->data['order_id'], C('pp_pro_order_status_id'), $message, false);
