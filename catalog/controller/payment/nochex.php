...
class ControllerPaymentNochex extends Controller {
	protected function index() {
        ...
        // The merchant ID is usually your Nochex registered email address but can be altered for "Merchant" accounts see below
	   	if (C('nochex_email') != C('nochex_merchant')){ // This MUST be changed on your Nochex account!!!!
            $this->data['merchant_id'] = C('nochex_merchant');
        } else {
			$this->data['merchant_id'] = C('nochex_email');
		}
		...
		$this->data['test'] = C('nochex_test');
        $this->data['success_url'] = U('checkout/success', '', 'SSL');
        $this->data['cancel_url'] = U('checkout/payment', '', 'SSL');
        $this->data['declined_url'] = U('payment/nochex/callback', 'method=decline', 'SSL');
        $this->data['callback_url'] = U('payment/nochex/callback', '&order=' . $this->session->data['order_id'], 'SSL');

		$this->render('payment/nochex.tpl');
	}
	...
	public function callback() {
		...
		if (isset($this->request->get['method']) && $this->request->get['method'] == 'decline') {
			$this->session->data['error'] = L('error_declined');
		...
		if (!$order_info) {
			$this->session->data['error'] = L('error_no_order');
		...
		if (strcmp($response, 'AUTHORISED') == 0) {
			$this->model_checkout_order->confirm($order_id, C('nochex_order_status_id'));
		} else {
			$this->model_checkout_order->confirm($order_id, C('config_order_status_id'), 'Auto-Verification step failed. Manually check the transaction.');
		}

		// Since it returned, the customer should see success.
		// It's up to the store owner to manually verify payment.
		$this->redirect(U('checkout/success', '', 'SSL'));
	}
}
?>