...
class ControllerPaymentPPProUK extends Controller {
	protected function index() {
		...
		$this->data['text_credit_card'] = L('text_credit_card');
		$this->data['text_start_date'] = L('text_start_date');
		$this->data['text_issue'] = L('text_issue');
		$this->data['text_wait'] = L('text_wait');

		$this->data['entry_cc_owner'] = L('entry_cc_owner');
		$this->data['entry_cc_type'] = L('entry_cc_type');
		$this->data['entry_cc_number'] = L('entry_cc_number');
		$this->data['entry_cc_start_date'] = L('entry_cc_start_date');
		$this->data['entry_cc_expire_date'] = L('entry_cc_expire_date');
		$this->data['entry_cc_cvv2'] = L('entry_cc_cvv2');
		$this->data['entry_cc_issue'] = L('entry_cc_issue');
		...
		$this->render('payment/pp_pro_uk.tpl');
	}
	...
	public function send() {
		...
		if (!C('pp_pro_uk_transaction')) {
			$payment_type = 'A';
		} else {
			$payment_type = 'S';
		}

		$request  = 'USER=' . urlencode(C('pp_pro_uk_user'));
		$request .= '&VENDOR=' . urlencode(C('pp_pro_uk_vendor'));
		$request .= '&PARTNER=' . urlencode(C('pp_pro_uk_partner'));
		$request .= '&PWD=' . urlencode(C('pp_pro_uk_password'));
		$request .= '&TENDER=C';
		...
		if ($response_info['RESULT'] == '0') {
			$this->model_checkout_order->confirm($this->session->data['order_id'], C('config_order_status_id'));
			...
			$this->model_checkout_order->update($this->session->data['order_id'], C('pp_pro_uk_order_status_id'), $message, false);

			$json['success'] = U('checkout/success');
		} else {
			switch ($response_info['RESULT']) {
				case '1':
				case '26':
					$json['error'] = L('error_config');
					break;
				case '7':
					$json['error'] = L('error_address');
					break;
				case '12':
					$json['error'] = L('error_declined');
					break;
				case '23':
				case '24':
					$json['error'] = L('error_invalid');
					break;
				default:
					$json['error'] = L('error_general');
					break;
			}
		}
