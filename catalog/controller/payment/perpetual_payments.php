...
class ControllerPaymentPerpetualPayments extends Controller {
	protected function index() {
		...
		$this->render('payment/perpetual_payments.tpl');
	}
	...
	public function send() {
		...
		$payment_data = array(
			'auth_id'        => C('perpetual_payments_auth_id'),
			'auth_pass'      => C('perpetual_payments_auth_pass'),
			...
			'tran_testmode' => C('perpetual_payments_test'),
			'tran_type'     => 'Sale',
			'tran_class'    => 'MoTo',
		);
		...
		if ($response) {
			...
			if (isset($data[0]) && $data[0] == 'A') {
				...
				if (isset($data[1])) {
					$message .= L('text_transaction') . ' ' . $data[1] . "\n";
				}

				if (isset($data[2])) {
					if ($data[2] == '232') {
						$message .= L('text_avs') . ' ' . L('text_avs_full_match') . "\n";
					} elseif ($data[2] == '400') {
						$message .= L('text_avs') . ' ' . L('text_avs_not_match') . "\n";
					}
				}

				if (isset($data[3])) {
					$message .= L('text_authorisation') . ' ' . $data[3] . "\n";
				}

				$this->model_checkout_order->update($this->session->data['order_id'], C('perpetual_payments_order_status_id'), $message, false);
