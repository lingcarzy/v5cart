...
class ControllerPaymentSagepayDirect extends Controller {
	...
	public function send() {
        ...
        $data['Vendor'] = C('sagepay_direct_vendor');
		...
		$data['Description'] = substr(C('config_name'), 0, 100);
		...
		$data['TxType'] = C('sagepay_direct_transaction');
		...
		if ($data['Status'] == '3DAUTH') {
			...
			$json['TermUrl'] = U('payment/sagepay_direct/callback');
			...
			$this->model_checkout_order->update($this->session->data['order_id'], C('sagepay_direct_order_status_id'), $message, false);
	...
	public function callback() {
		if (isset($this->session->data['order_id'])) {
			...
			if ($data['Status'] == 'OK' || $data['Status'] == 'AUTHENTICATED' || $data['Status'] == 'REGISTERED') {
				...
				$this->redirect(U('checkout/checkout', '', 'SSL'));
			}
		} else {
