...
class ControllerPaymentAuthorizeNetSim extends Controller {
	protected function index() {
		...
		$this->data['action'] = C('authorizenet_sim_url');
		 ...
		 */
		$data['x_login'] = C('authorizenet_sim_merchant');
		...
		$this->render('payment/authorizenet_sim_index.tpl');
	}
	...
	protected function calculateFpHash() {
		...
		$fp_hash = $hash->hmac_md5($code,
			C('authorizenet_sim_transaction_key') );
	...
	protected function calculateResponseHash() {
		...
		$code = C('authorizenet_sim_response_key') .
	...
	public function callback() {
		...
		$data['button_confirm'] = L('button_continue');
		...
		if ($data['hash_match'] ) {
			...
			if ($order_info) {
				if ($data['x_response_code'] == '1') {
					$this->model_checkout_order->confirm($order_id, C('authorizenet_sim_order_status_id'));
				} else {
		...
		$this->render('payment/authorizenet_sim_callback.tpl');
	}
}
?>