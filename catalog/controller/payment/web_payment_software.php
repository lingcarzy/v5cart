...
class ControllerPaymentWebPaymentSoftware extends Controller {
	protected function index() {
		...
		$this->render('payment/web_payment_software.tpl');
	}
	...
	public function send() {
		...
		$request  = 'MERCHANT_ID=' . urlencode(C('web_payment_software_merchant_name'));
		$request .= '&MERCHANT_KEY=' . urlencode(C('web_payment_software_merchant_key'));
		$request .= '&TRANS_TYPE=' . urlencode(C('web_payment_software_method') == 'capture' ? 'AuthCapture' : 'AuthOnly');
		...
		if (C('web_payment_software_mode') == 'test') {
			$request .= '&TEST_MODE=1';
		}
		...
		//If successful log transaction in V5Cart system
		if ('00' === (string)$xml->response_code) {
			...
			$this->model_checkout_order->update($this->session->data['order_id'], C('web_payment_software_order_status_id'), $message, false);
