...
class ModelInstall extends Model {
	public function mysql($data) {
		...
		$file = DIR_APPLICATION . 'v5cart.sql';
		...
		if ($sql = file($file)) {
			...
			foreach($sql as $line) {
				...
				if (($sql != '') && (substr($tsl, 0, 2) != "--") && (substr($tsl, 0, 1) != '#')) {
					...
					if (preg_match('/;\s*$/', $line)) {
						$query = str_replace("DROP TABLE IF EXISTS `v5_", "DROP TABLE IF EXISTS `" . $data['db_prefix'], $query);
						$query = str_replace("CREATE TABLE `v5_", "CREATE TABLE `" . $data['db_prefix'], $query);
						$query = str_replace("INSERT INTO `v5_", "INSERT INTO `" . $data['db_prefix'], $query);
			...
			mysql_query("INSERT INTO `" . $data['db_prefix'] . "setting` SET `group` = 'config', `key` = 'config_url', value = '" . mysql_real_escape_string(HTTP_V5CART) . "'", $connection);
