...
final class Loader {
	...
	public function library($library, $instance = false) {
		...
		if (file_exists($file)) {
			...
			if ($instance) {
				...
				$this->registry->set($library, new $class($this->registry));
			}
		} else {
	...
	public function model($model, $varname = null) {
		$file  = DIR_APPLICATION . 'model/' . $model . '.php';
		$class = 'Model' . preg_replace('/[^a-zA-Z0-9]/', '', $model);
		
		if (file_exists($file)) {
			include_once($file);
			if (!$varname) $varname = 'model_' . str_replace('/', '_', $model);
			$this->registry->set($varname, new $class($this->registry));
		} else {
