	...
	private $code;
  	...
  	public function __construct($registry) {
		...
		if (isset($this->session->data['affiliate_id'])) { 
			$affiliate_query = $this->db->query("SELECT * FROM @@affiliate WHERE affiliate_id = '" . (int)$this->session->data['affiliate_id'] . "' AND status = '1'");
			...
			if ($affiliate_query->num_rows) {
				...
				$this->code = $affiliate_query->row['code'];
      			...
      			$this->db->query("UPDATE @@affiliate SET ip = '" . ES($this->request->server['REMOTE_ADDR']) . "' WHERE affiliate_id = '" . (int)$this->session->data['affiliate_id'] . "'");
			} else {
				$this->logout();
			}
  		}
  	...
  	public function login($email, $password) {
		$affiliate_query = $this->db->query("SELECT * FROM @@affiliate WHERE email = '" . ES($email) . "' AND (password = SHA1(CONCAT(salt, SHA1(CONCAT(salt, SHA1('" . ES($password) . "'))))) OR password = '" . ES(md5($password)) . "') AND status = 1 AND approved = 1");
