...
class ModelReportReturn extends Model {
	public function getReturns($data = array()) {
		...
		if (!empty($data['filter_date_start'])) {
			$sql .= " AND DATE(r.date_added) >= '" . ES($data['filter_date_start']) . "'";
		}

		if (!empty($data['filter_date_end'])) {
			$sql .= " AND DATE(r.date_added) <= '" . ES($data['filter_date_end']) . "'";
		}
	...
	public function getTotalReturns($data = array()) {
		...
		switch($group) {
			case 'day';
				$sql = "SELECT COUNT(DISTINCT DAY(date_added)) AS total FROM `@@return`";
				break;
			default:
			case 'week':
				$sql = "SELECT COUNT(DISTINCT WEEK(date_added)) AS total FROM `@@return`";
				break;
			case 'month':
				$sql = "SELECT COUNT(DISTINCT MONTH(date_added)) AS total FROM `@@return`";
				break;
			case 'year':
		...
		if (!empty($data['filter_date_start'])) {
			$sql .= " AND DATE(date_added) >= '" . ES($data['filter_date_start']) . "'";
		}

		if (!empty($data['filter_date_end'])) {
			$sql .= " AND DATE(date_added) <= '" . ES($data['filter_date_end']) . "'";
		}
