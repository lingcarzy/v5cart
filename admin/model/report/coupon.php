...
class ModelReportCoupon extends Model {
	public function getCoupons($data = array()) {
		...
		if (!empty($data['filter_date_start'])) {
			$implode[] = "DATE(c.date_added) >= '" . ES($data['filter_date_start']) . "'";
		}

		if (!empty($data['filter_date_end'])) {
			$implode[] = "DATE(c.date_added) <= '" . ES($data['filter_date_end']) . "'";
		}
	...
	public function getTotalCoupons($data = array()) {
		...
		if (!empty($data['filter_date_start'])) {
			$implode[] = "DATE(date_added) >= '" . ES($data['filter_date_start']) . "'";
		}

		if (!empty($data['filter_date_end'])) {
			$implode[] = "DATE(date_added) <= '" . ES($data['filter_date_end']) . "'";
		}
