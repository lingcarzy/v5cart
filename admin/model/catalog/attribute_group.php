...
class ModelCatalogAttributeGroup extends Model {
	public function addAttributeGroup($data) {
		...
		foreach ($data['attribute_group_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO  @@attribute_group_description SET attribute_group_id = " . (int)$attribute_group_id . ", language_id = " . (int)$language_id . ", name = '" . ES($value['name']) . "'");
		}
	}
	...
	public function getAttributeGroup($attribute_group_id) {
		return $this->db->queryOne("SELECT * FROM  @@attribute_group WHERE attribute_group_id = " . (int)$attribute_group_id);
	}

	public function getAttributeGroups($data = array()) {
		$sql = "SELECT * FROM  @@attribute_group ag LEFT JOIN  @@attribute_group_description agd ON (ag.attribute_group_id = agd.attribute_group_id) WHERE agd.language_id = " . (int)C('config_language_id');

		if (!empty($data['filter_name'])) {
			$sql .= " AND agd.name LIKE '" . ES($data['filter_name']). "%'";
		}
		$sort_data = array(
			'agd.name',
			'ag.sort_order'
	...
	public function getTotalAttributeGroups() {
      	return $this->db->queryOne("SELECT COUNT(*) AS total FROM  @@attribute_group");
	}
}
?>