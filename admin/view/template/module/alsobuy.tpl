...
<div id="content">
  ...
  <div class="box">
    ...
    <div class="content">
      <form action="<?php echo UA('module/alsobuy'); ?>" method="post" enctype="multipart/form-data" id="form">
        <table id="module" class="list">
          ...
          <tbody id="module-row<?php echo $module_row; ?>">
            <tr>
              <td class="left"><input type="text" name="alsobuy_module[<?php echo $module_row; ?>][limit]" value="<?php echo $module['limit']; ?>" size="1" /></td>
              <td class="left"><input type="text" name="alsobuy_module[<?php echo $module_row; ?>][image_width]" value="<?php echo $module['image_width']; ?>" size="3" />
                <input type="text" name="alsobuy_module[<?php echo $module_row; ?>][image_height]" value="<?php echo $module['image_height']; ?>" size="3" />
                <?php if (isset($error_image[$module_row])) { ?>
                <span class="error"><?php echo $error_image[$module_row]; ?></span>
                <?php } ?></td>
              <td class="left"><select name="alsobuy_module[<?php echo $module_row; ?>][layout_id]">
			  <?php echo form_select_option($layouts, $module['layout_id'], null, 'layout_id', 'name'); ?>
                </select></td>
              <td class="left"><select name="alsobuy_module[<?php echo $module_row; ?>][position]">
			  <?php echo form_select_option($_['option_position'], $module['position'], true); ?>
                </select></td>
              <td class="left"><select name="alsobuy_module[<?php echo $module_row; ?>][status]">
                  <?php echo form_select_option($_['option_statuses'], $module['status'], true);?>
                </select></td>
				<td class="left">
					<select name="alsobuy_module[<?php echo $module_row; ?>][template]">
						<option value="">Default</option>
						<?php echo form_select_option($templates, $module['template']); ?>
					</select>
				</td>
              <td class="right"><input type="text" name="alsobuy_module[<?php echo $module_row; ?>][sort_order]" value="<?php echo $module['sort_order']; ?>" size="3" /></td>
...
function addModule() {	
	...
	html += '  <tr>';
	html += '    <td class="left"><input type="text" name="alsobuy_module[' + module_row + '][limit]" value="5" size="1" /></td>';
	html += '    <td class="left"><input type="text" name="alsobuy_module[' + module_row + '][image_width]" value="80" size="3" /> <input type="text" name="alsobuy_module[' + module_row + '][image_height]" value="80" size="3" /></td>';		
	html += '    <td class="left"><select name="alsobuy_module[' + module_row + '][layout_id]">';
	<?php foreach ($layouts as $layout) { ?>
	html += '      <option value="<?php echo $layout['layout_id']; ?>"><?php echo addslashes($layout['name']); ?></option>';
	<?php } ?>
	html += '    </select></td>';
	html += '    <td class="left"><select name="alsobuy_module[' + module_row + '][position]">';
	...
	html += '    </select></td>';
	html += '    <td class="left"><select name="alsobuy_module[' + module_row + '][status]">';
    html += '      <option value="1" selected="selected"><?php echo $_['text_enabled']; ?></option>';
    html += '      <option value="0"><?php echo $_['text_disabled']; ?></option>';
    html += '    </select></td>';
	html += '    <td class="left"><select name="alsobuy_module[' + module_row + '][template]"><option value="">Default</option>';
	<?php foreach ($templates as $template) { ?>
		html += '      <option value="<?php echo $template; ?>"><?php echo $template; ?></option>';
	<?php } ?>
	html += '    </select></td>';
	html += '    <td class="right"><input type="text" name="alsobuy_module[' + module_row + '][sort_order]" value="" size="3" /></td>';
