...
<div id="content">
  ...
  <div class="box">
    ...
    <div class="content">
      <form action="<?php echo UA('module/special'); ?>" method="post" enctype="multipart/form-data" id="form">
          ...
          <tbody id="module-row<?php echo $module_row; ?>">
            <tr>
              ...
              <td class="left"><select name="special_module[<?php echo $module_row; ?>][status]">
				...
				<td class="left"><select name="special_module[<?php echo $module_row; ?>][cache]">
					<?php echo form_select_option($_['option_statuses'], $module['cache'], true);?>
                </select>&nbsp;<input type="text" name="special_module[<?php echo $module_row; ?>][expire]" value="<?php echo isset($module['expire']) ? $module['expire'] : 3600; ?>" size="3" /> /s</td>
				<td class="left">
					<select name="special_module[<?php echo $module_row; ?>][template]">
						<option value="">Default</option>
						<?php echo form_select_option($templates, $module['template']); ?>
					</select>
				</td>
				<td class="left"><input type="text" name="special_module[<?php echo $module_row; ?>][id]" value="<?php echo $module['id']; ?>" size="3" /></td>
...
function addModule() {	
    ...
    html += '    </select></td>';
	html += '    <td class="left"><select name="special_module[' + module_row + '][cache]">';
    html += '      <option value="1" selected="selected"><?php echo $_['text_enabled']; ?></option>';
    html += '      <option value="0"><?php echo $_['text_disabled']; ?></option>';
    html += '    </select> <input type="text" name="special_module['+ module_row +'][expire]" value="3600" size="3" /> /s</td>';
	html += '    <td class="left"><select name="special_module[' + module_row + '][template]"><option value="">Default</option>';
	<?php foreach ($templates as $template) { ?>
		html += '      <option value="<?php echo $template; ?>"><?php echo $template; ?></option>';
	<?php } ?>
	html += '    </select></td>';
	html += '    <td class="left"><input type="text" name="special_module[' + module_row + '][id]" value="0" size="3" /></td>';
