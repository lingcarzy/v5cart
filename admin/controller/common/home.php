...
class ControllerCommonHome extends Controller {
	public function index() {
		...
		// Check install directory exists
 		if (is_dir(dirname(DIR_APPLICATION) . '/install')) {
			$this->data['error_install'] = L('error_install');
		} else {
		...
		if (!file_exists($file)) {
			$this->data['error_image'] = sprintf(L('error_image'). DIR_IMAGE);
		} else {
		...
		if (!file_exists($file)) {
			$this->data['error_image_cache'] = sprintf(L('error_image_cache'). DIR_IMAGE . 'cache/');
		} else {
		...
		if (!file_exists($file)) {
			$this->data['error_cache'] = sprintf(L('error_image_cache'). DIR_CACHE);
		} else {
		...
		if (!file_exists($file)) {
			$this->data['error_download'] = sprintf(L('error_download'). DIR_DOWNLOAD);
		} else {
		...
		if (!file_exists($file)) {
			$this->data['errorlogs'] = sprintf(L('error_logs'). DIR_LOGS);
		} else {
    	...
    	foreach ($results as $result) {
			...
			$action[] = array(
				'text' => L('text_view'),
				'href' => UA('sale/order/info', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'], 'SSL')
			);
	...
	public function chart() {
		...
		$data['order']['label'] = L('text_order');
		$data['customer']['label'] = L('text_customer');
		...
		switch ($range) {
			case 'day':
				for ($i = 0; $i < 24; $i++) {
					$query = $this->db->query("SELECT COUNT(*) AS total FROM `@@order` WHERE order_status_id > " . (int)C('config_complete_status_id') . " AND (DATE(date_added) = DATE(NOW()) AND HOUR(date_added) = '" . (int)$i . "') GROUP BY HOUR(date_added) ORDER BY date_added ASC");
